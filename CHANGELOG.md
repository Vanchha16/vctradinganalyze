...
Both pipelines' `run()` now return a small per-provider result object (`NewsIngestionResult`/`CalendarIngestionResult`, each carrying `list[ProviderOutcome]`) instead of a bare count/tuple, and log a provider failure at `ERROR` (mirroring `market_data_service.py`'s established structured-logging shape). **If every configured provider fails, `run()` raises** `AllNewsProvidersFailedError`/`AllEconomicCalendarProvidersFailedError` - both already existed in the exception hierarchy, unused, since before this phase. The scheduled Celery task lets it propagate (task reports FAILED, not a clean success); the admin-triggered manual refresh (`POST /admin/news`, `POST /admin/maintenance`) catches it and returns a clean `400`, not a fake zero-count `200`. Per-provider resilience is unchanged - one provider failing never stops the others

`GET /admin/system` gains `news`/`economic_calendar` fields: configured provider name(s), whether the active provider is a mock, last successful ingestion timestamp, and the last error if the most recent run failed - stored in Redis (`app/services/ingestion_health.py`, same `record`/`get` shape `rate_limit.py`/`quota.py` already use for small counters, no new migration), fail-open on read like every other liveness check this endpoint already makes. Mock usage is now also logged explicitly at startup (both the FastAPI app and the Celery worker), never silently discovered later

A new manual, read-only operator diagnostic, `backend/scripts/diagnose_ingestion.py`, reports configured providers, whether each required API key is present (never the value), the result of exactly one real fetch attempt per provider with the full error on failure, and current stored counts. **The actual production root cause is not fixed here** - the leading hypothesis (NewsAPI's free Developer plan rejecting a deployed server) is stated, not confirmed; the operator runs this script against the real box to find out. Finnhub activation for the calendar remains config-only, unchanged. See **ADR-139** for the full design

### Added - Phase 9F: Admin Symbol (Asset) Management (ADR-138)

Admins can now control which symbols the bot collects data for, analyses, and generates signals on, entirely from the UI - no more direct database access. Five new endpoints (`GET/POST /admin/assets`, `PATCH /admin/assets/{id}`, `POST /admin/assets/{id}/activate`, `POST /admin/assets/{id}/deactivate`), `require_admin`-gated, matching `GET /admin/users`' pagination convention exactly. `AdminAssetService` mirrors `AdminUserService`'s shape (constructor-injected repositories, one method per use case, private `_audit`/`_commit` helpers); `AssetRepository.list_filtered`/`count_filtered` gained an additive `search` param, no second filtering implementation

**Symbol is immutable after creation** - it's the natural key every `price_candle`/`indicator_result`/`smc_event` row is tied to via `asset_id`, and provider symbol mapping keys off it (docs/41). An attempt to change it via `PATCH` returns a clear `422`, not a silent no-op. **No hard delete** - `Asset` cascades to candles/indicators/SMC events; deactivation (`is_active=False`) is the only "remove it" action

`Asset.is_active` was already the single control point for three production pipelines (market data collection, hourly AI signal generation, news matching) - this phase is the missing admin surface for that existing lever, making it the operational cost/blast-radius control for the whole automated system. Deactivating a symbol stops all three immediately, no deploy needed. **Operator note: deactivating all but one symbol is now the supported way to run a low-cost single-symbol test.** Existing open (`ACTIVE`/`TRIGGERED`) signals are left to resolve naturally on deactivation, never auto-cancelled

New Admin Assets page (`/admin/assets`) reuses every existing admin table/filter-bar/dialog/confirmation component - no new UI primitives. Deactivate is behind a confirmation whose copy names exactly what stops. No migration - `Asset.is_active`/`symbol` uniqueness/every repository read method already existed; only write endpoints and a UI were missing. See **ADR-138** for the full design

### Fixed - Phase 9E: Signal Entry Confirmation - Live Production Defect (ADR-137)

**A live production defect is fixed**: `Signal` status was evaluated against Stop Loss/Take Profit the instant a signal existed, never confirming price had actually reached `entry_price` first. Every production signal so far closed at -1.0R within minutes, without the trade ever being entered (real 2026-08-07 XAU/USD example in ADR-137). `TRIGGERED` (reserved since ADR-088, never wired up) now gates this correctly: `ACTIVE` signals are pending orders, only ever checked for entry-trigger (`candle.low <= entry_price <= candle.high`, `entry_touched` in `app/services/signal_monitoring_service.py`); SL/TP are now only evaluated once `TRIGGERED`. A same-candle gap that touches both entry and stop loss is triggered-then-stopped-out, never left dangling. A new `signal_triggered_ttl_hours` setting (default 168h) closes a `TRIGGERED` signal that never resolves, read-time-only via `status_resolver.effective_status`, same treatment as `EXPIRED`. `_has_open_signal` (`workers/signal_tasks.py`, ADR-125's dedup gate) now also treats `TRIGGERED` as open, so a filled pending order can't silently reopen regeneration. No Telegram message is sent on a bare `TRIGGERED` transition. `backend/scripts/cancel_stale_active_signals.py` (dry-run by default, `--confirm` to write) is provided for the operator to cancel pre-deploy `ACTIVE` signals that were never entry-confirmed under the old semantics

### Fixed - Phase 9E: Telegram Price Precision (ADR-137)

Forex signal prices (entry/SL/TP) now render at 5 decimal places (3 for JPY-quoted pairs), not a single hardcoded 2dp for every asset - a forex stop loss fractions of a pip from entry previously rendered identically to entry (`"1.15"` for both, P&L `"-0.00"`). Metal/crypto/index unchanged at 2dp. Display only - the stored `Numeric(20, 8)` `Decimal` is untouched (`message_sections._format_price`). Frontend's `formatPrice` was checked and already uses a 2-5 fraction-digit range, not a hardcoded 2dp - no frontend change needed

### Changed - Phase 9E: Signal Generation Cadence Now a Setting (ADR-137)

The hourly signal-generation interval (`workers/signal_tasks.py`) moved from a hardcoded module constant into `settings.signal_generation_interval_seconds` (default unchanged, 3600s), so the operator can retune it via `.env` + restart without a code change/deploy. The interval itself is not changed in this phase - entry confirmation is expected to sharply reduce false-instant-stop-out message volume on its own; cadence is revisited after a week of real post-fix data (BACKLOG.md §21)

### Added - Phase 9D: Measurement, `GET /metrics` (ADR-136)

`GET /metrics` (Prometheus text format via `prometheus_client`, newly added to `backend/uv.lock`/`pyproject.toml`) - `http_requests_total{method,route,status}` and `http_request_duration_seconds{method,route}` for every request (`app/middleware/metrics.py`), plus the client library's default process/GC collectors. **Labeled by matched route template, never raw path** - verified directly that two requests to the same route with different path params produce one series with the count summed, not two, avoiding unbounded per-symbol cardinality on the 911MB production box. Fail-closed access control (`app/dependencies/metrics_auth.py`): `METRICS_AUTH_TOKEN` unset (default) or wrong/missing returns `404`, not `403`/an empty `200`, so an unconfigured deployment doesn't advertise the route's existence - a static bearer token, not `require_admin`, so this keeps working when the database is unhealthy. Excluded from 9A's per-IP rate limiting and from its own request metrics. `scripts/local_env.py` sets a fixed dev token so the endpoint is usable locally out of the box; `.env.example` documents the real setting as a placeholder only. Unblocks `GET /admin/system`'s (ADR-116) previously-gated telemetry limitation. Celery worker/queue metrics and surfacing this in the Admin UI are recorded as follow-ups (BACKLOG.md §3), not built here. See **ADR-136** for the full design and docs/60 §8 for scope boundaries
...
`--cov-fail-under=90` added to the `backend` job's `pytest` invocation, enforcing docs/06 §21's already-documented 90% coverage goal (BACKLOG.md §4) against the 93% Phase 9C measured. CI-only - `pytest` run locally without the flag stays gate-free

**`uv` turned out to be available in this environment after all** (`python -m uv`, not on `PATH` directly) - contrary to Phase 9C's finding. Ran `uv lock` and regenerated `backend/uv.lock` for real: added `pytest-cov`/`coverage` (closing the gap Phase 9C's build report flagged), and **fixed a second, previously-unnoticed staleness** - `httpx` was promoted from a dev-only to a runtime dependency in `pyproject.toml` back in Phase 3B, but the lock file still listed it only under `dev-dependencies`, never in the main package's `dependencies` list, until this relock. `email-validator` remains genuinely not installed (it was never added to `pyproject.toml` at all, not a lock-staleness case) - unchanged, out of this phase's scope

**Found in the process, unrelated to this phase's scope, not fixed here**: `uv run ruff check .` and `uv run mypy app` both currently fail on `main` (pre-existing `E501` line-length violations in `tests/test_telegram_message_sections.py`; a pre-existing `Awaitable[Any]` comparison type error in `app/dependencies/rate_limit.py`/`quota.py`) - confirmed present before this phase's changes via `git stash`. The `backend` CI job would fail on push regardless of the coverage-gate/lock work here. Flagged loudly per the build spec; fixing pre-existing lint/type debt outside Items A-C was judged out of scope for this phase
