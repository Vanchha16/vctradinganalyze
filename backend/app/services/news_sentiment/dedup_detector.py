...
def is_duplicate(candidate: RawNewsArticle, existing_articles: Sequence[NewsArticle]) -> bool:
    ...
    for existing in existing_articles:
        ...
        if title_similarity(candidate.title, existing.title) >= _TITLE_SIMILARITY_THRESHOLD:
            # existing.published_at comes back naive on SQLite;
            # candidate.published_at is freshly built by a provider
            # (already aware) - without normalizing both, calling the
            # pipeline a second time against a non-empty table raises
            # TypeError (found running POST /admin/news twice, Phase 7D-C).
            candidate_published_at = as_aware_utc(candidate.published_at)
            existing_published_at = as_aware_utc(existing.published_at)
