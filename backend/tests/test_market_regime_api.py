...
def test_get_market_regime_returns_structured_evidence(
    ...
    response = client.get(
        f"/api/v1/analysis/market-regime/{asset.symbol}", params={"timeframe": "h1"}
    )
    ...
    assert set(body["confidence_breakdown"].keys()) == {
        "trend_clarity",
        "volatility_clarity",
    ...
    # Evidence only - no trading recommendation/strategy-compatibility fields.
    assert "recommendation" not in body
    assert "compatible_strategies" not in body
    assert "strategy" not in body


def test_get_market_regime_case_insensitive_symbol(client: TestClient, session: Session) -> None:
    asset = _make_asset(session)
    _seed_trending_candles(session, asset, Timeframe.H1, 300, drift=0.3)

    lower = client.get("/api/v1/analysis/market-regime/eurusd", params={"timeframe": "h1"})
    upper = client.get("/api/v1/analysis/market-regime/EURUSD", params={"timeframe": "h1"})

    assert lower.status_code == 200
    assert upper.status_code == 200


def test_get_market_regime_returns_404_for_unknown_asset(client: TestClient) -> None:
    response = client.get("/api/v1/analysis/market-regime/NOTREAL", params={"timeframe": "h1"})

    assert response.status_code == 404
    assert response.json()["error"] == "resource_not_found"


def test_get_market_regime_returns_404_when_no_candles(
    client: TestClient, session: Session
) -> None:
    asset = _make_asset(session)
...
def test_get_market_regime_rejects_invalid_timeframe(client: TestClient, session: Session) -> None:
    asset = _make_asset(session)

    response = client.get(
        f"/api/v1/analysis/market-regime/{asset.symbol}", params={"timeframe": "not-a-timeframe"}
    )

    assert response.status_code == 422


def test_get_market_regime_requires_authentication(client: TestClient, session: Session) -> None:
    ...
    response = client.get(
        "/api/v1/analysis/market-regime/XAUUSD", params={"timeframe": "h1"}
    )

    assert response.status_code == 401
def test_get_market_regime_multi_timeframe_returns_combined_verdict(
    client: TestClient, session: Session
) -> None:
    asset = _make_asset(session)
    ...
    response = client.get(f"/api/v1/analysis/market-regime/{asset.symbol}/multi-timeframe")
...
def test_get_market_regime_multi_timeframe_handles_partial_data(
    client: TestClient, session: Session
) -> None:
    asset = _make_asset(session)
