...
`risk_review` (ADR-167) is the AI risk manager's verdict on a BUY/SELL: a second AI call that may only approve or veto, with reasons and the single biggest risk.
- **`mode: "shadow"`:** the verdict was recorded and changed nothing.
- **`mode: "enforce"`:** a veto made `recommendation` `"wait"`.
- **`null`:** for WAIT, when `AI_RISK_REVIEW_MODE` is `off`, or when the review call failed. A failure adds a warning and never blocks the analysis.
