# AI Reasoning Engine

Version: 1.1

Implementation (Phase 6A): docs/50_AI_ORCHESTRATOR_ARCHITECTURE.md, ADR-077 through ADR-084. This document's "AI Reasoning Engine" and docs/07's "AI Orchestrator" are implemented as one combined `AIOrchestratorEngine` class (ADR-077), not two separate components. §6's BUY/SELL/WAIT rules are implemented as a concrete deterministic decision tree in `recommendation_decision.py` (ADR-078) - the AI never decides the recommendation, only narrates it. §8's evidence-priority ordering (Risk > Economic > SMC > Technical > News) is realized as `recommendation_decision.py`'s hard-reject precedence (Risk Engine's veto checked first, per ADR-014) plus the deterministic evidence bundle's field ordering in the prompt - not a numeric weighting scheme.

---

# 1. Objective

The AI Reasoning Engine transforms structured evidence into a professional market analysis.

The AI never performs technical calculations.

The AI never calculates indicators.

The AI never detects Smart Money Concepts.

The AI only reasons over validated evidence produced by other engines.

---

# 2. Responsibilities

The AI shall:

- Explain market conditions
- Explain why a signal exists
- Detect conflicting evidence
- Explain uncertainty
- Recommend BUY / SELL / WAIT
- Explain confidence
- Explain risk
- Generate professional reports

---

# 3. Inputs

The AI receives ONLY structured data.

Example

Technical Engine

SMC Engine

Economic Engine

News Engine

Risk Engine

Confidence Engine

Market Regime Engine

Current Price

Timeframe

Asset

---

# 4. AI Must NEVER

Invent indicators

Invent prices

Invent support

Invent resistance

Invent economic events

Invent news

Invent confidence

Invent market structure

Guess missing information

Ignore conflicts

---

# 5. Reasoning Framework

Step 1

Understand Market Context

↓

Step 2

Analyze Trend

↓

Step 3

Analyze Momentum

↓

Step 4

Analyze Market Structure

↓

Step 5

Analyze News

↓

Step 6

Analyze Economic Events

↓

Step 7

Analyze Risk

↓

Step 8

Resolve Conflicts

↓

Step 9

Generate Recommendation

↓

Step 10

Explain Decision

---

# 6. Recommendation Rules

BUY

Only when

Bullish evidence dominates

Risk acceptable

No critical conflicts

SELL

Only when

Bearish evidence dominates

Risk acceptable

No critical conflicts

WAIT

Required when

Evidence conflicts

Confidence too low

Major economic event approaching

Extreme volatility

Missing confirmation

---

# 7. Explainability

Every recommendation MUST explain

Trend

Momentum

Market Structure

SMC

News

Economic Events

Risk

Confidence

Warnings

---

# 8. Evidence Priority

Highest

Risk

Economic Events

SMC

Technical Analysis

News

Lowest

Minor Indicators

---

# 9. Conflict Resolution

Example

Technical

BUY

SMC

SELL

News

BUY

Economic

SELL

Risk

HIGH

Recommendation

WAIT

Reason

Conflicting institutional evidence.

---

# 10. Hallucination Prevention

AI cannot answer when

Input missing

Evidence inconsistent

Critical engines unavailable

Instead respond

"Insufficient evidence to generate a reliable recommendation."

---

# 11. Confidence Explanation

Confidence must reference

Trend alignment

Indicator agreement

SMC confluence

Economic support

News support

Risk quality

Market regime

---

# 12. Reasoning Style

Professional

Objective

Evidence-based

Concise

No emotional language

No hype

No promises

---

# 13. Output Template

Market Summary

Trend Analysis

Technical Analysis

SMC Analysis

Economic Analysis

News Analysis

Risk Analysis

Recommendation

Confidence

Warnings

Conclusion

---

# 14. Example Output

Recommendation

BUY EURUSD

Confidence

89%

Market Summary

The broader trend remains bullish across the Daily and H4 timeframes.

Technical Analysis

EMA alignment is bullish with strong ADX confirming trend strength.

SMC Analysis

A fresh bullish Order Block and confirmed Break of Structure support continuation.

Economic Analysis

Recent CPI data favors weaker USD expectations.

News Analysis

News sentiment remains moderately bullish.

Risk Analysis

No major economic releases within the next four hours.

Risk remains Medium.

Warnings

Strong resistance approximately 35 pips above current price.

Conclusion

Current evidence supports a BUY opportunity with disciplined risk management.

---

# 15. Logging

Store

Prompt Version

Input Hash

Evidence

Output

Execution Time

Confidence

Warnings

---

# 16. Future Enhancements

Personalized AI

Multi-Agent Debate

Self-Evaluation

Historical Learning

Adaptive Reasoning

Confidence Calibration

Chain-of-Verification