...
API Keys Editable From the Admin UI: Encrypted at Rest, Write-Only,
Super-Admin Only
...
Rotating an API key meant SSHing to the box and editing `.env`. The
operator asked to do it from the website.

ADR-154 had just rejected exactly this shape for Admin Settings, on the
grounds that DB-backed configuration creates a second source of truth and
that Celery caches config at import. Re-checking the call sites showed
the second objection was **wrong**: every key is read at call or
construction time, not into a module-level constant.
`dependencies/market_data.py`, `news.py` and `telegram.py` build their
providers per request and per Celery task; `openai_provider` reads on
each request. So a DB-backed lookup propagates without restarting
anything.

The first objection stands and is answered by scope: this is not a
key/value store for arbitrary settings, it is a fixed list of four
integrations.

Decision

`api_credentials`, one row per managed key, with a resolver that reads
**database first, environment second**.

**A separate table from `system_settings`**, deliberately. That table
holds plain operational bookkeeping (the Telegram polling cursor) and is
read without ceremony. These are credentials. Sharing a table would
invite a future `get_by_key()` that hands a secret to a caller that only
wanted a setting.

**Encrypted at rest** with Fernet, master key in `CREDENTIAL_ENCRYPTION_KEY`.
The honest boundary: this defends against a leaked database dump -
`~/deploy_backups` accumulates one per deploy - and against read access
to Postgres. It does **not** defend against someone who already has the
server, since they hold the master key too. Operator chose this over
plaintext.

**Write-only.** No route returns a stored key; there is no reveal
endpoint anywhere. The UI shows whether a key is set, its source, and the
last four characters. Four characters of a forty-character key is the
difference between "which key is this?" and guesswork, not a disclosure.
The audit log records the same hint and never the value - otherwise every
rotated key would live permanently in a table the admin UI can read.

**Super-admin only**, matching the precedent that role changes are
(ADR-122). A web form that writes credentials raises the blast radius of
a compromised admin account, and that is the guard the operator chose.

**`source` is reported explicitly** - `database`, `environment` or
`unset`. Without it, a key working fine from `.env` would look identical
to a stored one and Clear would appear to do nothing. DELETE means "stop
overriding `.env`", which is the only way back once a key is stored, and
is deliberately not the same as saving an empty string (rejected with a
422).

Consequences

Keys can be rotated from the browser, taking effect within the resolver's
30-second cache window, with no deploy and no restart. Nothing changes
for an existing deployment: with no rows present every caller gets the
`.env` value it always got, which is also the fallback if the database is
unreachable - the resolver fails open rather than taking market data down.

New runtime dependency: `cryptography`. `CREDENTIAL_ENCRYPTION_KEY` must
be generated per environment; without it the feature reports itself
disabled in the UI rather than failing on submit, and everything
continues to run off `.env`.

A rotated master key makes stored values undecryptable. `decrypt` raises
rather than returning a fallback, because a silently-wrong key would be
sent to a vendor and produce a confusing 401 far from the real cause.
Recovery is to clear the affected rows and re-enter them.

**A migration hazard worth recording.** `alembic revision --autogenerate`
for this table also emitted `drop_constraint` for
`uq_telegram_accounts_link_code` and `uq_telegram_accounts_user_id`. Both
were stripped before shipping.

Verified against production afterwards rather than assumed: uniqueness
there is enforced by unique **indexes** (`ix_telegram_accounts_link_code`,
`ix_telegram_accounts_user_id`) - what SQLAlchemy renders for
`unique=True, index=True`. No constraint by either `uq_` name exists.
Those names live only in the local SQLite dev database, where an earlier
`batch_alter_table` rebuild materialised the same uniqueness as named
constraints; autogenerate diffed that against the model.

The failure mode was therefore a `DROP CONSTRAINT` on something that does
not exist - an error aborting the migration mid-deploy - rather than a
silent loss of uniqueness. Production integrity was never at risk. The
lesson is unchanged and now twice-proven: **never ship an autogenerated
migration from this repo without reading it**, because the local SQLite
database and production Postgres do not agree about `telegram_accounts`.

Future Review

If a fifth integration is added, `credential_resolver.FALLBACK_SETTING`
