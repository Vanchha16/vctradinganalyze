...
`uq_telegram_accounts_link_code` and `uq_telegram_accounts_user_id`.
Those constraints exist in production and are declared on the model; the
"difference" is an artifact of the local SQLite dev database, where an
earlier `batch_alter_table` rebuild lost their *names*. Applying it would
have silently dropped real uniqueness guarantees - letting one user link
two Telegram accounts. The generated migration was hand-corrected and the
reason recorded in its docstring. **Never ship an autogenerated migration
from this repo without reading it.**
