...
- `get_current_user` (`app/dependencies/auth.py`): extracts the bearer token, decodes it, verifies `type == "access"`, and loads the `User` via `UserService.get_user_by_id`. **Since Phase 9B (ADR-133), it also rejects a user whose `is_active` is `False` or whose `deleted_at` is set**, reusing `InactiveAccountException` - a deliberate narrowing of the previous "authenticate only" contract, made necessary once Phase 8C gave admins the ability to disable/soft-delete an account, which a live access token would otherwise outlive for up to its full lifetime. It still performs **no other authorization checks** (`is_verified`, role) — those remain out of scope for this phase
