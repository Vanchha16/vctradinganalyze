...
- The interval for each timeframe's task matches that timeframe (e.g. the M1 task runs every minute, the H1 task every hour) - not an arbitrary fixed polling interval for everything, since over-polling a D1 asset every minute wastes provider quota for no benefit, and under-polling M1 defeats the point of the timeframe.
